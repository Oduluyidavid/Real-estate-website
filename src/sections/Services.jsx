      ...
      <section id='services' className={`${darkMode ? 'dark bg-gray-800' : 'light bg-red-100'} 
       lg:w-[95%] w-full h-fit m-auto rounded-xl flex flex-col justify-center items-start lg:px-20 px-6 py-10 gap-10`}>
        <div className='flex flex-col justify-center items-start gap-4'>
          ...
          <h1 data-aos='zoom-in' className='text-black text-[40px] font-semibold leading-10 dark:text-white'>Top real estate <br></br> Services avaliable</h1>
        </div>
        <div id='service-box' className='grid lg:grid-cols-3 grid-cols-1 justify-center items-center gap-8'>
          {
            service.map((item, index)=>(
              <div data-aos='zoom-in' data-aos-delay='200' key={index} className='bg-white dark:bg-black h-[350px] px-8 py-16 
              flex flex-col justify-center items-start rounded-xl border-b-[5px] border-red-600 hover:bg-red-300 cursor-pointer'>
